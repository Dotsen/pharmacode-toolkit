...
Beyond this matrix, the 0.2.0 decoder was checked by hand on six publicly
available sample images (rendered barcodes with captions, tight crops, one with
a rule drawn across the bars, one symbology chart). They exposed the defects
fixed in 0.2.0 and are not redistributed here; they are not a benchmark, and no
scanned or photographed printed code has been tested.
